  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        // Sort orders by creation date (latest first) and pick the latest 5
        const latestOrders = allOrders
          .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
          .slice(0, 5);

        setRows(latestOrders);
      } catch (error) {
        ...
        <TableBody>
          {rows.map((row) => (
            <TableRow key={row.id}>
              ...
              <TableCell className="tableCell">${row.total_amount}</TableCell>
              <TableCell className="tableCell">{row.payment_status}</TableCell>
              <TableCell className="tableCell">
                <span className={`status ${row.status}`}>{row.status}</span>
              </TableCell>
            </TableRow>
